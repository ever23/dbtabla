...
class prosesingSql
{
    ...
    */
    busqueda(cadena,campo_bus,campos,joins,where,group,having,order,limit)
    {
        ...
        arrSql.push("FROM")
        arrSql.push(this.__helpers.__protectIdentifiers(this.tabla))
    ...
    */
    delete(where)
    {
        return `DELETE FROM ${this.__helpers.__protectIdentifiers(this.tabla)} ${this.__booleanSql("WHERE",where)};`
    }
    /**
    ...
    */
    update(sets,where,error)
    {
        ...
        return `UPDATE ${this.__helpers.__protectIdentifiers(this.tabla)} SET ${col.slice(0,-1)} ${this.__booleanSql("WHERE",where)};`
    }
    /**
    ...
    insert(campos)
    {
        ...
        if(!(campos instanceof Array))
        {
            for(let coll of this.colums)
            {
                ...
                let verify=(coll.autoincrement || coll.defaultNull) && 
                (
                    campos[coll.name]===undefined || campos[coll.name]===null || 
                    (
                        typeof campos[coll.name]==="string" && 
                        (
                            campos[coll.name]==="undefined" || campos[coll.name].toLowerCase()==="null"
                        )
                    )
                )
                if(!(verify || (campos[coll.name]===undefined && coll.default!==undefined && coll.default!=="")))
                {
        ...
        }else
        {
            ...
            for(let i in this.colums)
            {
                ...
                let verify=(coll.autoincrement || coll.defaultNull) 
                && (campos[i]===undefined || campos[i]===null || (typeof campos[i]==="string" && (campos[i]==="undefined"  || campos[i].toLowerCase()==="null")))
                if(!(verify || (campos[i]===undefined && coll.default!==undefined && coll.default!=="")))
                {
                    attrs.push(coll.name)
    ...
    */
    __booleanSql(type,sql)
    {
        ...
        {
            return  type+" " +this.__helpers.__object2boleandSql(sql)
    ...
    */
    __campos(campos)
    {
        ...
        {
            return  this.__helpers.__campos([ this.tabla + ".*"])
        }
        return  this.__helpers.__campos(campos)
    }
    /**
    ...
    */
    __resolveParamsSelect(campos,joins,where,group,having,order,limit)
    {
        ...
        if(typeof where==="number" || (typeof where==="string" && (/^[\d]+$/.test(where) || /^( {0,}Limit )/i.test(where))))
        {
            limit=where
            where = undefined
            group = undefined
            ...
            $ah = having
            $ao = order
        }else if (/^( {0,}group by )/i.test(where))
        {
        ...
        if(typeof group==="number" || (typeof group==="string" && (/^[\d]+$/.test(group) || /^( {0,}LIMIT )/i.test(group))))
        {
            limit=group
            group = undefined
            having = undefined
        ...
        }else if (typeof group==="string" && /^( {0,}having )/i.test(group))
        {
            having = group
            order = $ah
            limit = $ao
            group = undefined
        } else if (typeof group==="string" && /^( {0,}ORDER by )/i.test(group))
        {
            order = group
            limit = $ao
            group = undefined
        ...
        if(typeof having==="number" || (typeof having==="string" && (/^[\d]+$/.test(having) || /^( {0,}LIMIT )/i.test(having))))
        {
            limit=having
            having = undefined
            order = undefined
            $ah = having
            $ao = order
        }else if (typeof having ==="string"  && /^( {0,}ORDER BY )/i.test(having))
        {
            order = having
            limit = $ao
            having = undefined
        ...
        if(typeof order==="number" || (typeof order==="string" && (/^[\d]+$/.test(order) || /^( {0,}LIMIT )/i.test(order))))
        {
            limit=order
            order = undefined
            $ao = order
