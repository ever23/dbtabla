const dbHelpers = require("tabla-model/sqlHelpers")
const SQLITE3_DB = require("./const").SQLITE3_DB
/**
* prosesingSql
...
class prosesingSql {
    ...
    */
    #campos(campos) {
        if (!(campos instanceof Array) || campos.length === 0) {
    ...
    */
    #limit(limit) {
        if (/^[\s]*LIMIT[\s]+/i.test(limit)) {
            ...
            let L = limit.slice(0, 2).map(n => parseInt(n))
            return "LIMIT " + L.join(",")
        }
