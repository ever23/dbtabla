...
class prosesingSql
{
    ...
    */
    busqueda(cadena,campo_bus,campos,joins,where,group,having,order,limit)
    {
        ...
        let arrSql=[],
            ...
            w=this.__where(params.where),
            g=this.__groupBy(params.group),
            h=this.__having(params.having),
            o=this.__orderBy(params.order),
            l=this.__limit(params.limit)
            
        if(!w)
        {
            w=this.__where(`(${search})>1`)
        }else {

            w =this.__where(`(${w.replace(/^[\s]*where[\s]+(.*)/i,"$1")}) AND (${search})>1`)
        }
        if(!o)
        {
    ...
    */
    __where(where)
    {
        ...
        {
            return  "WHERE " +this.__helpers.__object2boleandSql(where)
        }else if(where!==undefined && where!==null)
        {
            return "WHERE " +where.replace(/^([\s]*WHERE[\s]+(.*))/i,"$1")
            //where=where
        }
        return false
    }
    ...
    */
    __having(having)
    {
        ...
        {
            return "HAVING " +this.__helpers.__object2boleandSql(having)
        }else if(having!==undefined && having!==null)
        {
             return "HAVING " +having.replace(/^([\s]*HAVING[\s]+(.*))/i,"$1")
        }
        return false
    }
    /**
