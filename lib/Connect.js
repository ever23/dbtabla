...
class Connect
{
    ...
    *
    * @param {string} table - nombre de la tabla
    * @param {function} resolve - funcion a ejecutar cuando se obtengan los metadatos
    * @param {boolean} create - existencia en la base de datos
    *
    * @return {boolean}
    */
    inModel(table,resolve,create)
    {
        if(!this.model(table))
            return false
        if(create)
        {
            this.__createTable(this.model(table))
                .then(resolve).catch(e=>{throw e})
        }else
        {
            resolve(this.model(table).getData())
        }
        return true
